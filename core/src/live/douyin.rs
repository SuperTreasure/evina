...
async fn get_real_id(rid: String) -> Result<HashMap<&'static str, String>, Box<dyn Error>> {
    ...
    // 去除结尾 / 返回去除后的地址
    let mut id = if rid.clone().ends_with("/") {
...
async fn phone(cli: Cli, id: String, client: Client) -> Result<String, Box<dyn Error>> {
    match client
        .get(format!("https://v.douyin.com/{}", id))
        ...
        Ok(resp) => match resp.url().query() {
            Some(_) => {
                ...
                match client
                    ...
                    .header(
                        COOKIE,
                        local_cookie(cli.config_file, String::from("cookie_douyin")).unwrap(),
                    )
                    ...
                    .await
                {
                    Ok(resp) => match resp.text().await {
                        Ok(data) => {
                            match re_match(r#"https://live.douyin.com/(\d+)"#, &data.clone()) {
                                Some(id) => Ok(id),
                                None => {
                                    return Err(format!(
                                        "抖音 | {}: 短链接失效，请填写新的短链接",
                    ...
                    Ok(cookie) => {
                            ...
                            .await
                        {
                            Ok(resp) => match resp.json::<Value>().await {
                                Ok(result) => match result["data"]["data"][0]["stream_url"]
                                    ["live_core_sdk_data"]["pull_data"]["stream_data"]
                                    .as_str()
                                {
                                    Some(stream_data) => {
                                        let stream_data_json: Value =
                                            serde_json::from_str(stream_data)?;
                                        let rtmp = match &stream_data_json["data"]["origin"] {
                                            Value::Object(origin) => {
                                                origin["main"]["flv"].to_string()
                                            }
                                            Value::Bool(_) => todo!(),
                                            Value::Number(_) => todo!(),
                                            Value::String(_) => todo!(),
                                            ...
                                            Value::Null => match &stream_data_json["data"]["ao"] {
                                                ...
                                                Value::Array(_) => todo!(),
                                                Value::Null => {
                                                    match &stream_data_json["data"]["uhd"] {
                                                        Value::Object(origin) => {
                                                            origin["main"]["flv"].to_string()
                                                        }
                                                        Value::Bool(_) => todo!(),
                                                        ...
                                                        Value::Array(_) => todo!(),
                                                        Value::Null => {
                                                            match &stream_data_json["data"]["hd"] {
