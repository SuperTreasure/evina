...
pub async fn get_real_id(rid: String) -> Result<HashMap<&'static str, String>, Box<dyn Error>> {
    let client = Client::builder().connect_timeout(time::Duration::from_secs(20)).build()?;
    let mut map = HashMap::new();
    match rid.as_str() {
        i if i.contains("http") => Err(format!("ID格式错误, 仅支持房间号的方式").into()),
        _ => {
            let id = String::from("https://m.douyu.com/") + &rid.clone();
            let response = match client.get(id.clone()).header(reqwest::header::USER_AGENT, get_user_agent()).send().await {
            ...
            match response.text().await {
                Ok(text) => {
                    ...
                    let real_id = match re_match(r#"rid":(\d{1,8}),"vipId"#, &text.clone()) {
                        Some(data) => data,
                        None => return Err(format!("斗鱼 | {}: 真实ID获取失败或找不到直播间", rid).into()),
                    };
                    // 得到直播间名称。
    ...
    match id {
        Some(id) => match get_real_id(id.clone()).await {
            Ok(map) => {
                ...
                let url = String::from("https://www.douyu.com/swf_api/homeH5Enc?rids=") + &map["id"];
                let response = match client.get(url).header(reqwest::header::USER_AGENT, get_user_agent()).send().await {
                    Ok(response) => response,
                    Err(e) => return Err(e.into()),
                };
                // match response.text().await {
                match response.json::<Value>().await {
                    Ok(home_h5_enc) => {
                        let text = if let Value::Object(ref data_map) = home_h5_enc["data"] {
                            if let Some((_first_key, first_value)) = data_map.iter().next() {
                                first_value.to_string().replace("\\\"", "\"")
                            } else {
                                eprintln!("The data object is empty.");
                                exit(1);
                            }
                        } else {
                            eprintln!("Expected 'data' to be a JSON object.");
                            exit(1);
                        };
                        let result = match re_match(r#"(vdwdae325w_64we[\s\S]*function ub98484234[\s\S]*?)function"#, &text) {
                            Some(data) => data,
                            None => return Err(format!("斗鱼 | {}: 未找到直播间,请检查ID", id).into()),
                        };
                        let fun = regex::Regex::new(r"eval.*?;}").unwrap().replace_all(&result, "strc;}").to_string();
                        ...
                        let rb = {
                            let mut h = Md5::new();
                            h.update(format!("{}10000000000000000000000000001501{}{}", map["id"], tt, v));
                        ...
                        let data = run_js(&format!(r#"{func_sign};sign({},10000000000000000000000000001501,{})"#, map["id"], tt));
                        ...
                        let response = match client
                            .post(format!("https://www.douyu.com/lapi/live/getH5Play/{}", map["id"]))
                            .form(&params)
