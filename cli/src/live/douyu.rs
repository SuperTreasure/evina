...
use super::{
    util::{get_user_agent, re_match, run_js},
    Information,
};
use chrono::Local;
use md5::{Digest, Md5};
        ...
        Some(id) => match get_real_id(id.clone()).await {
            Ok(map) => {
                ...
                match response.text().await {
                    Ok(text) => {
                        let result =
                            match re_match(r#"(vdwdae325w_64we[\s\S]*function ub98484234[\s\S]*?)function"#, &text) {
                                Some(data) => data,
                        ...
                        let func_sign =
                            regex::Regex::new(r"return rt;}\);?").unwrap().replace_all(&res, "return rt;}").to_string();

                        let func_sign = func_sign.replace(r#""(function "#, "function sign");
                        let func_sign = func_sign.replace(r#";""#, ";");
                        let func_sign = func_sign.replace(r#"}""#, "};");
                        let func_sign =
                            func_sign.replace("CryptoJS.MD5(cb).toString()", format!(r#""{}""#, &rb).as_str());
