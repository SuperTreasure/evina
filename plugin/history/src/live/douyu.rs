...
pub async fn down_his(rid: Option<String>, date: Option<NaiveDate>) {
    ...
    match rid {
        Some(id) => {
            match date {
                Some(date) => {
                    match get_real_id(id.clone()).await {
                        Ok(map) => {
                                        ...
                                        Ok(response) => {
                                            match response.json::<Value>().await {
                                                Ok(result) => {
                                                    ...
                                                    let show_time: DateTime<Local> =
                                                        DateTime::from_timestamp(room["show_time"].as_i64().unwrap(), 0).unwrap().into();
                                                    let end_time: DateTime<Local> =
                                                        DateTime::from_timestamp(room["end_time"].to_string().replace(r#"""#, "").parse::<i64>().unwrap(), 0)
                                                            .unwrap()
                                                            .into();
                                                    let nowtime: DateTime<Local> =
                                                        DateTime::from_timestamp(room["nowtime"].as_i64().unwrap(), 0).unwrap().into();
                                                    ...
                                                    let end_time = NaiveDateTime::new(end, NaiveTime::from_hms_opt(0, 0, 0).unwrap())
                                                        .and_local_timezone(Shanghai)
                                                        .unwrap()
                                                        .timestamp();
                                                    url = format!("https://v.douyu.com/wgapi/vod/center/authorShowVideoList?page=1&limit=5&up_id={}&start_time={}&end_time={}", up_id, start_time, end_time);
                                                    match client.get(url).header(USER_AGENT, get_user_agent()).send().await {
                                                        Ok(response) => {
                                                            match response.json::<Value>().await {
                                                                Ok(result) => {
                                                                        ...
                                                                        false => {
                                                                            for list in result["data"]["list"].as_array().unwrap() {
                                                                                ...
                                                                                for list2 in video_list.as_array().unwrap() {
                                                                                    ...
                                                                                    match client
                                                                                    .get(format!("https://v.douyu.com/wgapi/vod/center/getShowReplayList?vid={}&up_id={}", hash_id, up_id))
                                                                                    ...
                                                                                    for list in result["data"]["list"].as_array().unwrap() {
                                                                                        ...
                                                                                        match get_sign(hash_id.clone(), vid.clone()).await {
                                                                                            Ok(mut params) => {
                                                                                                ...
                                                                                                match client
                                                                                                    .post("https://v.douyu.com/wgapi/vodnc/front/stream/getStreamUrlWeb")
                                                                                                    .form(&params)
                                                                                                    .header(USER_AGENT, get_user_agent())
                                                                                                    .send()
                                                                                                    .await
                                                                                                {
                                                                                                    Ok(response) => match response.json::<Value>().await {
                                                                                                        Ok(result) => {
                                                                                                            let path = Path::new(&cli.download_dir)
                                                                                                                .join(format!("{}--录播下载", nickname))
                                                                                                                .join(date.to_string())
                                                                                                                .join(show_remark);
                                                                                                            let super_url =
                                                                                                            ...
                                                                                                            match client.get(super_url.clone()).header(USER_AGENT, get_user_agent()).send().await {
                                                                                                                    ...
                                                                                                                    Ok(text) => {
                                                                                                                        for line in text.lines() {
                                                                                                                            if !line.trim().is_empty() && !line.trim().starts_with('#') {
                                                                                                                                ...
                                                                                                                                let down_path = path.join(format!("{}--{}.ts", show_remark, name));
...
pub fn aaaa() {
    ...
    // 打印结果
    println!("昨天的日期是：{}", yesterday.format("%Y-%m-%d"));
}
